...
resource "azurerm_kubernetes_cluster_node_pool" "linux101" {
  ...
  vm_size               = "Standard_B2s" # Smaller instance type
  ...
  max_count    = var.default_node_pool_enable_auto_scaling ? 1 : null
  min_count    = var.default_node_pool_enable_auto_scaling ? 1 : null
  node_count   = 1 # Reduced node count
